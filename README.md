...
SSH'd in the backend, pull the updated backend files from GitHub and restart the service:
```bash
git pull
sudo systemctl restart gddl-recommender
```
...
```bash
cd extension
zip -r ../gddl-demon-recommender.zip .
```
